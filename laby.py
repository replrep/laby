...
class Game:
    ...
    def make_field(self):
        ...
        while row < self.height or col < self.width:
            if (random.random() < 0.5 or col == self.width) and row < self.height:
    ...
    def make_wall_n(self, row, col):
        ...
        if (
            ...
            and self.field[row - 1][col + 1] == 0
            and self.field[row - 2][col] == 0
        ):
    ...
    def make_char(self):
        ...
        while (
            ((row % 2) == 0 and (col % 2) == 0)
            or self.is_wall_pos(row, col)
            or self.field[row][col] != 0
        ):
            row = random.randint(1, self.height - 1)
            col = random.randint(1, self.width - 1)
    ...
    def move_player_n(self):
        if (
            self.player_row > 0
            and self.field[self.cell_to_field(self.player_row) - 1][
                self.cell_to_field(self.player_col)
            ]
            != Game.WALL
        ):
    ...
    def move_player_s(self):
        if (
            self.player_row < self.cellrows - 1
            and self.field[self.cell_to_field(self.player_row) + 1][
                self.cell_to_field(self.player_col)
            ]
            != Game.WALL
    ...
    def move_player_w(self):
        if (
            self.player_col > 0
            and self.field[self.cell_to_field(self.player_row)][
                self.cell_to_field(self.player_col) - 1
            ]
            != Game.WALL
        ):
    ...
    def move_player_e(self):
        if (
            self.player_col < self.cellcols - 1
            and self.field[self.cell_to_field(self.player_row)][
                self.cell_to_field(self.player_col) + 1
            ]
            != Game.WALL
    ...
    def check_collision(self):
        if (
            ...
            self.field[self.cell_to_field(self.player_row)][
                self.cell_to_field(self.player_col)
            ] = 0
            self.view.update()
        if (
...
class View:
    def __init__(self, main_window, game):
        ...
        endbutton = tk.Button(
            frame, text="Ende", command=self.quit_app, bg="white", relief="flat"
        )
    ...
    def configure_handler(self, event):
        ...
        self.smiley = self.smiley_original_size.subsample(
            min(
                int((self.smiley_original_size.width()) / (0.5 * self.cellwidth)),
                int((self.smiley_original_size.height()) / (0.5 * self.cellheight)),
            )
        )
        self.home = self.home_original_size.subsample(
            min(
                int((self.home_original_size.width()) / (0.3 * self.cellwidth)),
                int((self.home_original_size.height()) / (0.3 * self.cellheight)),
            )
        )
    ...
    def update(self):
        ...
        self.store.create_text(
            ...
            anchor=tk.W,
            font=("Helvetica", -int(self.cellheight * 0.8), "bold"),
        )

        self.labyrinth.delete(tk.ALL)
        self.labyrinth.create_rectangle(1, 1, self.width - 2, self.height - 2, width=5)
        ...
        if self.game.player_char != "":
            self.labyrinth.create_text(
                ...
                font=("Helvetica", -int(self.cellheight * 0.4), "bold"),
                tags="player_char",
            )
        ...
        for row in range(self.game.height):
            for col in range(self.game.width):
                if (
                    self.game.is_vertical(row, col)
                    and self.game.field[row][col] == Game.WALL
                ):
                    cell_x = int((col + 1) / 2)
                    cell_y = int(row / 2)
                    self.labyrinth.create_line(
                ...
                if (
                    self.game.is_horizontal(row, col)
                    and self.game.field[row][col] == Game.WALL
                ):
                    cell_x = int(col / 2)
                    cell_y = int((row + 1) / 2)
    ...
    def show_word(self):
        ...
        self.labyrinth.create_text(
            ...
            font=("Helvetica", -int(self.cellheight * 2), "bold"),
        )
